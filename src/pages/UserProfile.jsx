          ...
          <div className="primary w-full flex">
            ...
            >
              <div className="profile relative w-[250px] h-[200px] rounded-3xl">
                <img
                  src={userImg?.[1].url || defaultImage}
                  alt="coverphoto"
                  className="rounded-3xl w-full h-full object-cover"
                />
                <div className="overlay w-full h-full absolute bg-black top-0 left-0 opacity-40 rounded-3xl"></div>
                <img
                  src={userImg?.[0].url || defaultImage}
                  alt="profile"
                  className="w-[100px] outline-black outline-1 shadow-md h-[100px] md:w-[120px] md:h-[120px] absolute -bottom-8 object-cover border-4 border-white rounded-full"
                />
              </div>
            </motion.div>
            <div className="flex gap-2 w-[70%] items-center justify-evenly">
              ...
              >
                ...
                <p className="text-md mt-1">
                  MBTI (
                  <span className="text-colortext-primary font-bold mx-1">
                    {userData?.mbti}
                  </span>
                  )
                </p>
          ...
          <div className="info w-full flex flex-col gap-6">
            ...
            >
              <div className="interest flex flex-col gap-2">
                <h1 className="font-semibold text-colortext-primary">
                  {languageData.profilePage.interests}
                </h1>
                <ul className="flex flex-wrap gap-4">
                  {userData?.interests?.map((item) => (
                    <li className="bg-gray-100 p-2 px-3 rounded-md" key={item}>
                      {item}
                    </li>
                  ))}
                </ul>
            ...
            >
              <div className="location flex flex-col gap-2">
                <h1 className="font-semibold text-colortext-primary">
                  {languageData.profilePage.address}
                </h1>
                <p>
                  {userData?.city}, {userData?.locality},{" "}
            ...
            >
              <div className="dateOfBirth flex flex-col gap-2">
                <h1 className="font-semibold text-colortext-primary">
                  {languageData.profilePage.birthDate}
                </h1>
                ...
                userImg.map((img) => {
                  return (
                    <img
                      ...
                      alt={`${img.type}`}
                      key={img.title}
                      className="max-w-[400px] h-[200px] object-cover"
                    />
                  );
                })}
            </div>
