...
const Interest = () => {
  ...
  const addInterest = (interest) => {
    if (detailInfo.interests.includes(interest)) {
      const updatedInterests = detailInfo.interests.filter(
        (item) => item !== interest
      );
    ...
    <FadeCard>
      <div className="card w-full md:w-[700px] bg-colorbg-primary p-4 py-6 rounded-sm flex flex-col gap-8 overflow-hidden duration-200">
        <div className="heading flex flex-col gap-2">
          ...
          <span className="text-colortext-primary font-bold">
            {detailInfo.interests.length} / 5+
          </span>
          <p className="text-gray-500">
            Add at least 5 interests to your profile. You'll be able to discuss,
            engage, and meet like-minded souls in these communities.
          </p>
        </div>
        <hr />
        <div
          className="interest w-full flex flex-wrap gap-6 items-center justify-center h-[350px] md:h-[200px] overflow-y-scroll"
          style={{
            scrollbarWidth: "thin",
            scrollbarColor: "#4A5568 #CBD5E0",
            scrollbarTrackColor: "#CBD5E0",
          }}
        >
          {interests.map((interest, index) => (
            <span
              key={index}
              className={` ${
                detailInfo.interests.includes(interest)
                  ? "bg-btnbg-primary text-colortext-third"
                  : "text-[#707070] bg-colorbg-secondary"
              } duration-200 font-medium  p-1 px-3 cursor-pointer rounded-md`}
