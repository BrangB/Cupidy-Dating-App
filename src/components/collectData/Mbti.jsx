      ...
      <div className='flex flex-wrap gap-6 items-center justify-center overflow-y-scroll' style={{ scrollbarWidth: 'thin', scrollbarColor: '#4A5568 #CBD5E0', scrollbarTrackColor: '#CBD5E0' }}>
          {mbtiTypes.map((item, index) => (
            <div key={index} className='flex flex-col items-center justify-center gap-3'>
                <img src={item.img} alt="mbti" className={`${detailInfo.mbti == item.value ? 'border-btnbg-primary border-4' : ''} w-[80px] h-[80px] md:w-[100px] md:h-[100px] rounded-full object-cover cursor-pointer duration-150`} onClick={() => setDetailInfo({...detailInfo, mbti: item.value})}/>
                <span
